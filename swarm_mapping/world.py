...
class World:
    ...
    def update_agents_map(self, title=None, size=(5,5), fignum=21):
        ...
        for row in range(len(self.agents_map)):
            for col in range(len(self.agents_map[row])):
...
class Agent:
    def __init__(self, num, world, init_pos, init_vel, sensor_range, marker_size):
        ...
        self.agent_map = np.full((world.width, world.height), -2)
    ...
    def update_map(self):
        x, y = np.round(self.pos).astype(int)
        observation = self.world.state[y - self.range: y + self.range + 1,
                                       x - self.range: x + self.range + 1]

        # remove observation of self and other drones
